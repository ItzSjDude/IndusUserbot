...
async def _(event):
    ...
    if Config.PRIVATE_GROUP_BOT_API_ID is None:
        await eor(event,"Please set the required environment variable `PRIVATE_GROUP_BOT_API_ID` for this plugin to work")
        return
    ...
    if event.reply_to_msg_id:
        ...
        if input_str == "media":
            ...
            ievent = await eor(event,"Downloaded to {} in {} seconds.".format(downloaded_file_name, ms))
            ...
            try:
                ...
                await ievent.edit("ERROR: " + str(exc))
                os.remove(downloaded_file_name)
            else:
                end = datetime.now()
                ms_two = (end - start).seconds
                os.remove(downloaded_file_name)
                await ievent.edit("Uploaded to https://telegra.ph/{} in {} seconds.".format(media_urls[0], (ms + ms_two)), link_preview=True)
        elif input_str == "text":
