...
import time
from indus.utils import progress, humanbytes, time_formatter, admin_cmd, sudo_cmd, eor
...
@indus.on(admin_cmd(pattern="rnupload (.*)"))
@indus.on(sudo_cmd(pattern="rnup (.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    thumb = None
...
@indus.on(admin_cmd("rnstreamupload (.*)"))
@indus.on(sudo_cmd("rnstreamupload (.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    ...
    if event.reply_to_msg_id:
        ...
        if os.path.exists(downloaded_file_name):
            ...
            try:
                ...
                await ievent.edit("Downloaded in {} seconds. Uploaded in {} seconds.".format(ms_one, ms_two))
        else:
            await ievent.edit("File Not Found {}".format(input_str))
    else:
