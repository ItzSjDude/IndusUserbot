...
from indus import CMD_HELP, bot
...
async def kang(args):
    ...
    if message and message.media:
        if isinstance(message.media, MessageMediaPhoto):
            ...
            await await eor(args,f"`{random.choice(KANGING_STR)}`")
            photo = io.BytesIO()
        ...
        if len(splat) == 3:
            if char_is_emoji(splat[1]):
                if char_is_emoji(splat[2]):
                    return await eor(args,"check `.info stickers`")
...
@indus.on(admin_cmd(pattern="stkrinfo$"))
@indus.on(sudo_cmd(pattern="stkrinfo$", allow_sudo=True))
