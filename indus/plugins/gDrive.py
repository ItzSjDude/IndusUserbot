...
@indus.on(admin_cmd(pattern=r"ugdrive ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
...
@indus.on(admin_cmd(pattern=r"drivesch ?(.*)"))
async def sch(event):
    if event.fwd_from:
        return
    if CLIENT_ID is None or CLIENT_SECRET is None:
...
@indus.on(admin_cmd(pattern=r"gdrivedir ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
    if CLIENT_ID is None or CLIENT_SECRET is None:
...
async def upload_file(http, file_path, file_name, mime_type, event, parent_id):
    ...
    body = {
        "title": file_name,
        "description": "Uploaded using IndusUserBot",
        "mimeType": mime_type,
    }
    if parent_id is not None:
...
@indus.on(admin_cmd(pattern=r"gfolder ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
